...
## DBML 格式（資料模型）
用於描述實體關係模型 (ERM)，所有實體寫在同一個 `docs/spec/erm.dbml` 檔案中。
...
用於描述功能規格，採用三層式結構：Feature > Rule > Example。每個功能獨立一份文件於 `docs/spec/features/<中文功能簡稱>.feature`。
...
## DBML 格式要求
將資料模型以 DBML 格式輸出到 `docs/spec/erm.dbml`（所有實體寫在同一個檔案中）。
...
將每個 Feature 以及其涵蓋的 Rules/Examples，以 Gherkin Language 依序輸出到 `docs/spec/features/` 中。
