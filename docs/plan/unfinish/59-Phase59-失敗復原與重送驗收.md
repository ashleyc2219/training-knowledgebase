...
2. **真實 S3 上重跑多篇切點** —— 承接 P25 §11 那一列。主軸是 Phase 12 的 `a3_after_first_site_before_second`：**預期會重現 partial（A 新 B 舊）**；O3 是 FAIL，這裡只記錄觀察，不得改寫成通過、不得刪頁回滾、不得放寬 F49。**現況核對（2026-09-15，修正波）：** 實跑涵蓋的是 Phase 25 §8 的**五個**多篇切點（1、2、3、4a、4b），原文只點名 `a3` 一個；逐個切點的原文在 `docs/plan/report/recovery-20260915-0644.md` 附錄 A。其中只有 4a／4b（交易已提交、公開物件沒補完）走 `publishing.resume_publish`，切點 1／2／3 的復原是**以同一個 `operation_id` 重走完整路徑**（見該函式 docstring）。
