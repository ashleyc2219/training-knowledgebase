...
- 與本 Phase 有關的 O1–O7 gate 狀態：O5 未以真實帳號驗證前，FakeWriter 綠燈只代表決策邏輯正確，不代表 Bedrock 判斷節點可用；O7 未核定前，核定類別表的內容仍是待維護者確認的設定，不是已核定資料。
...
| 修改 | `src/training_kb/writing/prompts.py` | `prompt_classify_comment`：允許清單 + 不可信留言分區（檔案 owner 是 Phase 17，命名沿用 `prompt_<node>`）。 |
| 修改 | `infra/training_kb_stack.py` | 本階段讓 `training-kb-import` 開始呼叫模型，所以那支 Lambda 要補上一條 `bedrock:InvokeModel`（只限核定的生成模型 ARN）；Lambda 本體由 [Phase 42](./42-Phase42-Feedback與View固定匯入.md) 建立，這裡只加權限。 |
...
Phase 17：CommentClassification: dict[str, object]（required 只有 category）、prompts 的 <source_data> 分區與 _as_data 轉義
Phase 18：「走 correction？」對照表把 CommentClassification 列為「否」；生成參數 inference_config
Phase 42：validate_feedback、import_feedback、ImportResult、operation_id_for
```
...
- **留言是不可信資料，判斷參數不在這裡傳。** `prompt_classify_comment` 沿用 Phase 17 固定的 `<source_data>` 分區與轉義（不另創標記名稱），system 明說不得執行資料區的指示；設計 §14.3 的 `max_tokens=512`、`temperature=0.1` 由 Phase 18 的 `inference_config`（寫在 Phase 15 建立的 `writing/client.py`，對應 Bedrock `Converse` 的 `inferenceConfig`）統一設定，本階段只給 system／user／schema 三個輸入。
...
`Call` 用 `typing.NamedTuple`、`FakeWriter` 用 `dataclasses.dataclass`／`field`；`request_attempts` 就是實際送出的 request 數，`calls[0].system`／`.user` 留給 Step 4 的 prompt 斷言。`FakeWriter` 只實作 `generate_json`，足以當 `Writer` 用，因為本階段不碰 `embed` 與 `converse_with_tools`。`operation_id` 用 `operation_id_for("feedback", "f_50")` 的實際格式 `op-feedback-f_50`（00A 第 3.3 節），不要在測試裡自創短字串。
...
四個 prompt assertion 一併寫在 `tests/unit/test_feedback_category.py`（`tests/unit/test_prompts.py` 是 Phase 17 的檔案，本階段不動它）：`writer.calls[0].node == CLASSIFY_NODE`；`writer.calls[0].user.count("</source_data>") == 1`（留言裡偽造的結束標記被轉義成 `&lt;/source_data&gt;`）；允許清單含 `待分類`；prompt 不含評分、`user` ID 或其他回饋的文字。
...
- [ ] `COL` Rule 4、5、6 標為 primary 且各有直接 assertion；`ING` Rule 25 標為「相關（primary 在 Phase 31）」；未把 FakeWriter 綠燈說成 O5 已通過。
